...
function isValidEPSEmail(email) {
  if (!email) return false;
  // Supports both eastsidepre.org (as requested) and eastsideprep.org (actual domain)
  return /^[^\s@]+@(eastsidepre|eastsideprep)\.org$/i.test(email);
}
...
async function doLogin(role) {
  ...
  if (!isValidEPSEmail(email)) { showErr(role, 'Please use a valid @eastsideprep.org email address.'); return; }
...
function launchApp(role, email, token) {
  ...
  // Save to local storage for autofill
  localStorage.setItem('eps_user_email', email);
...
function restoreSession() {
  ...
  // Prefill login emails from local storage
  const savedEmail = localStorage.getItem('eps_user_email') || '';
  if (savedEmail) {
    const sEmail = document.getElementById('s-email');
    const fEmail = document.getElementById('f-email');
    const aEmail = document.getElementById('a-email');
    if (sEmail) sEmail.value = savedEmail;
    if (fEmail) fEmail.value = savedEmail;
    if (aEmail) aEmail.value = savedEmail;
  }
...
function openClaimModal(id) {
  ...
  const savedEmail = localStorage.getItem('eps_user_email') || '';
  document.getElementById('claimContact').value = savedEmail;
...
function confirmClaim() {
  ...
  if (!isValidEPSEmail(contact)) {
    toast('Please enter a valid @eastsideprep.org email address to claim.', 'err');
    return;
  }
...
// ─── SHEETS LOGGING (Updated to use fetch + JSON body) ──
async function logToSheet(eventType, item) {
  const data = {
    token: currentToken || '',
    event: eventType,
  ...
  console.log(`[EPS Retriever] Sending to Sheet:`, data);
  ...
  try {
    ...
    if (!result.ok) {
      ...
      toast('Error saving to sheet: ' + (result.error || 'Unknown error'), 'err');
    } else {
      console.log(`[EPS Retriever] Successfully logged ${eventType} for item ${item.id}`);
    }
  } catch (err) {
